...
import { textColor, backgroundColor, backgroundColorHeaderAndButton, backgroundColorHeaderAndButtonDim } from "./theme";

export const RoundedButtonMain = styled.button.attrs({
  className: "rounded-full w-full py-2 px-4 border-2"
})`
  color: ${textColor};
  ...
  &:hover {
    background-color: ${backgroundColorHeaderAndButtonDim};
    color: #000;
  }
  &:after {
    background-color: #b8b852;
  }
`;

export const RoundedButtonAlt = styled.button.attrs({
  className: props =>
    `rounded-full  py-2 px-4 ${props.margin} ${props.size} ${props.position} border-2`
})`
  color: ${textColor};
  background-color:${backgroundColorHeaderAndButton};
  transition: background 0.2s;
...
export const ListButton = styled.li.attrs({
  className:
    "p-4 px-10 mt-2 whitespace-no-wrap rounded-full text-xs md:text-base text-gray-600 hover:text-gray-800"
})`
  color: ${textColor};
  background-color:${backgroundColorHeaderAndButton};
  transition: background 0.2s;
...
//   --background: #141414;
//   --text-color: #efefef;
//   --button-icon: url('data:image/svg+xml,\
//   <svg version="1.1" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">\
...
//   </svg>');
//   --button-icon-filter: invert(0.61) sepia(0.8) saturate(5) hue-rotate(0deg) brightness(0.92);
