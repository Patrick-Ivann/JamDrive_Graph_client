            ...
            {data.prositsByPromo.filter(prosit => filterStore.filterStore.filterWord && filterPrositByNomProsit(prosit, filtre)).map(prosit => {
                ...
                return (
                    <div key={`divPrositPromo_${prosit._id}`}>
                        <IndivPrositForm key={`indivForm${prosit._id}`} nomProsit={prosit.nomProsit} unite={prosit.unite} prositId={prosit._id} ></IndivPrositForm>
                        <h4 key={`h4_${prosit._id}`}> {prosit.nomProsit.replace(/_/g, ' ').split(/(?=[A-Z])/).join(" ")} </h4>
                        <p key={`p_${prosit.allerFichier.id}`}><a key={`a_${prosit.allerFichier.id}`} href={prosit.allerFichier.path}>{prosit.allerFichier.nom} Aller</a></p>
                        <p key={`p_${prosit.retourFichier.id}`} ><a key={`a_${prosit.retourFichier.id}`} href={prosit.retourFichier.path}>{prosit.retourFichier.nom} Retour</a></p>
                        {
                            prosit.ressourceEleve.map((ressource, index) => <p key={`pRessourceEleve_${index}`} ><a key={ressource.id} href={ressource.path}>{ressource.nom} Ressoruce </a></p>)
                        }
